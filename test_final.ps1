<#
.SYNOPSIS
    Comprehensive test suite for PTK (Proxy Token Killer).
    Tests common usages of all supported PowerShell commands,
    calculates character counts and % savings, compares native vs PTK
    execution speed, and validates passthrough fallbacks.
#>
...
# 1. Locate newest PTK binary (prefers debug during development)
if (-not $BinaryPath) {
    ...
    if ((Test-Path $debug) -and (Test-Path $release)) {
        if ((Get-Item $debug).LastWriteTime -ge (Get-Item $release).LastWriteTime) {
            $BinaryPath = $debug
        } else {
            $BinaryPath = $release
        }
...
if (-not $BinaryPath -or -not (Test-Path $BinaryPath)) {
    Write-Error "[FAIL] ptk.exe not found! Please build it first using .\build.ps1"
    exit 1
}

Write-Host "=================================================================" -ForegroundColor Cyan
Write-Host "           PTK Automated Verification & Savings Benchmark         " -ForegroundColor Cyan
Write-Host "=================================================================" -ForegroundColor Cyan
...
function Run-ComparisonTest {
    ...
    # Execute Native (timed)
    $sw = [System.Diagnostics.Stopwatch]::StartNew()
    $nativeOut = pwsh -NoProfile -NonInteractive -Command "$NativeCommand" 2>&1 | Out-String
    $nativeMs = [math]::Round($sw.Elapsed.TotalMilliseconds, 1)
    $sw.Stop()

    # Execute PTK (timed)
    $sw.Restart()
    $ptkOut = pwsh -NoProfile -NonInteractive -Command "$PtkCommand" 2>&1 | Out-String
    $ptkMs = [math]::Round($sw.Elapsed.TotalMilliseconds, 1)
    $sw.Stop()

    $nativeChars = $nativeOut.Length
    $ptkChars = $ptkOut.Length

    $savingsPct = 0
    if ($nativeChars -gt 0) {
        $savingsPct = [math]::Round(((1.0 - ($ptkChars / [double]$nativeChars)) * 100), 1)
    }
    ...
    if ($ExpectFallback) {
        $nativeTrimmed = $nativeOut.Trim()
        $ptkTrimmed = $ptkOut.Trim()
    ...
    $results.Add([PSCustomObject]@{
        ...
        PTKChars       = $ptkChars
        SavingsPct     = if ($ExpectFallback) { "Fallback" } else { "$savingsPct%" }
        NativeMs       = $nativeMs
        PtkMs          = $ptkMs
...
Run-ComparisonTest -Category "Select-String" -TestName "Wildcard Multi-File (sls [package] *.toml)" `
    -NativeCommand "Select-String '[package]' Cargo.toml" -PtkCommand "& '$BinaryPath' sls '[package]' Cargo.toml"
...
Run-ComparisonTest -Category "Select-String" -TestName "Case Sensitive (sls -CaseSensitive)" `
    -NativeCommand "Select-String -CaseSensitive 'PTK' README.md" -PtkCommand "& '$BinaryPath' sls -CaseSensitive 'PTK' README.md"
...
Run-ComparisonTest -Category "EventLog" -TestName "EventLog List (-List)" `
    -NativeCommand "Get-EventLog -List" -PtkCommand "& '$BinaryPath' get-eventlog -List"
...
Run-ComparisonTest -Category "TestNet" -TestName "Ping Test (Test-NetConnection 127.0.0.1)" `
    -NativeCommand "Test-NetConnection 127.0.0.1" `
    -PtkCommand "& '$BinaryPath' test-net 127.0.0.1"

Run-ComparisonTest -Category "TestNet" -TestName "Alias check (tnc)" `
    -NativeCommand "Test-NetConnection 127.0.0.1" `
    -PtkCommand "& '$BinaryPath' tnc 127.0.0.1"

Run-ComparisonTest -Category "TestNet" -TestName "TCP Port Check (Failed port 80)" `
    -NativeCommand "Test-NetConnection 127.0.0.1 -Port 80" `
    -PtkCommand "& '$BinaryPath' test-net 127.0.0.1 -Port 80"

Run-ComparisonTest -Category "TestNet" -TestName "Direct ptk run 'tnc'" `
    -NativeCommand "Test-NetConnection 127.0.0.1 -Port 80" `
    -PtkCommand "& '$BinaryPath' run 'tnc 127.0.0.1 -Port 80'"

Run-ComparisonTest -Category "TestNet" -TestName "Fallback: Pipeline (| Select)" `
    -NativeCommand "Test-NetConnection 127.0.0.1 | Select-Object -ExpandProperty PingSucceeded" `
    -PtkCommand "& '$BinaryPath' run 'Test-NetConnection 127.0.0.1 | Select-Object -ExpandProperty PingSucceeded'" -ExpectFallback $true
...
Run-ComparisonTest -Category "JSON" -TestName "Direct ptk run convertfrom-json" `
    ...
    -PtkCommand "& '$BinaryPath' run 'ConvertFrom-Json -InputObject ''{\""status\"": \""ok\""}'''"

Run-ComparisonTest -Category "JSON" -TestName "Fallback: JSON pipeline" `
    -NativeCommand "'{\""id\"": 99}' | ConvertFrom-Json | Select-Object -ExpandProperty id" `
    -PtkCommand "& '$BinaryPath' run '''{\""id\"": 99}'' | ConvertFrom-Json | Select-Object -ExpandProperty id'" -ExpectFallback $true
...
Set-Content -Path $sampleCsv -Value "Id,Name,Role`n1,Alice,Admin`n2,Bob,Developer`n3,Charlie,Designer`n4,David,QA`n5,Eve,Manager"

Run-ComparisonTest -Category "CSV" -TestName "ConvertFrom-Csv string" `
    -NativeCommand "Get-Content '$sampleCsv' | ConvertFrom-Csv" `
    -PtkCommand "& '$BinaryPath' from-csv -InputObject (Get-Content '$sampleCsv' -Raw)"
...
Run-ComparisonTest -Category "CSV" -TestName "Fallback: CSV pipeline (| Measure)" `
    -NativeCommand "'id,val`n1,10`n2,20' | ConvertFrom-Csv | Measure-Object" `
    -PtkCommand "& '$BinaryPath' run '''id,val`n1,10`n2,20'' | ConvertFrom-Csv | Measure-Object'" -ExpectFallback $true

Remove-Item $sampleCsv -Force -ErrorAction SilentlyContinue
...
$results | Format-Table -AutoSize -Property Category, TestName, NativeChars, PTKChars, SavingsPct, NativeMs, PtkMs, SpeedRatio, Status, Notes
...
# ---------------------------------------------------------
# SPEED COMPARISON (native vs PTK)
# ---------------------------------------------------------
...
$slowerCount = ($results | Where-Object { $_.PtkMs -gt $_.NativeMs }).Count

Write-Host "`n-----------------------------------------------------------------" -ForegroundColor Cyan
Write-Host "                    SPEED COMPARISON SUMMARY                      " -ForegroundColor Cyan
Write-Host "-----------------------------------------------------------------" -ForegroundColor Cyan
Write-Host "Total wall time : native = $totalNativeMs ms | ptk = $totalPtkMs ms" -ForegroundColor Gray
Write-Host "Average per test: native = $avgNativeMs ms  | ptk = $avgPtkMs ms" -ForegroundColor Gray
Write-Host "Overall speed ratio (native/ptk): $overallRatio" -ForegroundColor Gray
Write-Host "Tests where PTK was slower than native: $slowerCount / $totalCount" -ForegroundColor $(if ($slowerCount -eq 0) { "Green" } else { "Yellow" })

if ($overallRatio -ge 1.0) {
    Write-Host "PTK is overall $([math]::Round((($overallRatio - 1.0) * 100), 1))% faster than native commands." -ForegroundColor Green
} else {
    Write-Host "PTK is overall $([math]::Round(((1.0 - $overallRatio) * 100), 1))% slower than native commands." -ForegroundColor Yellow
}
